...
function Email({ email, onFieldClick, selectedError, attemptedFields }) {
  const isFieldDisabled = (field) => {
    if (!selectedError) return false; // Return false if no error is selected
    const currentAttempts = attemptedFields?.[selectedError] || []; // Handle undefined safely
  ...
  return (
    <div className="email">
      ...
      <div
        ...
        onClick={() =>
          selectedError && !isFieldDisabled("header") && onFieldClick("header")
        }
      >
        {email.header}
      </div>
      ...
      <div
        ...
        onClick={() =>
          selectedError && !isFieldDisabled("body") && onFieldClick("body")
        }
      >
        {email.body}
      </div>
